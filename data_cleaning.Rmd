...
pacman::p_load(tidyverse, haven, magrittr, car, psych, binoculaR)
```
...
arab3 %<>% 
  ...
  # Dependent Variable
arab3 %<>% 
  mutate(islamistparties1 = ifelse(q518a2 > 5, NA, 5 - q518a2)) %>%             mutate(islamistparties2 = ifelse(q518b2 > 5, NA, 5 - q518b2)) %>% 
  mutate(islamistgov = ifelse(q5184 > 5, NA, 5 - q5184)) %>% 
  mutate(religinterfere = ifelse(q6061 > 5, NA, q6061)) %>% 
  mutate(religleaders = ifelse(q6062 > 5, NA, 5 - q6062)) %>% 
  mutate(religleadersinfl = ifelse(q6063 > 5, NA, 5 - q6063)) %>%
...
arab3$islamistparties<-pmax(arab3$islamistparties1,arab3$islamistparties2,na.rm=T)

araball <- plyr::rbind.fill(arab4,arab3)

table(ss$religparty)
table(ss$islamistgov)
table(arab4$q6063)
table(arab4$q605a)

Q605a. 1st sentence: I prefer a religious
political party over a non-religious p	1
2
3
4
5
98
99	I strongly agree with the 1st sentence
I agree with the 1st sentence
I agree with the 2nd sentence
I strongly agree with the 2nd sentence
