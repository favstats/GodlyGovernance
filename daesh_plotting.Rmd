...
goal_viol_alg <- plot_goal_viol(arab4, "Algeria", save = F, legend = F, labels = labels)
...
goal_viol_pal <- plot_goal_viol(arab4, "Palestine", save = F, legend = F)
...
ggsave(combined, file = "images/combined.png", height = 10, width = 9)
, top = "To what extent do you agree with the goals
of Daesh/ Daesh's use of violence?"

  arab_wide <- arab4 %>% 
#  filter(cntry == country) %>% 
  select(daesh_goal_num, daesh_violence_num, wt, cntry)  
...
ggsave(ss, file = "data/test.png", width = 6, height = 2)

legend <- get_legend(goal_viol_alg)

combined <- plot_grid(legend,
  goal_viol_alg, 
             goal_viol_jor, 
             goal_viol_leb, 
             goal_viol_mor, 
...
ggsave(combined, file = "images/combined.png", height = 10, width = 6)
cowplot::g
```
